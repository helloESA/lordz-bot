...
const memegen = async (tema , text1 , text2 = 'anjay') => {
    request.get(`https://api.memegen.link/images/${tema}/${text1}/_/${text2}`, function (error, response, body) {
        if (!error && response.statusCode == 200) {
        const databs64 = Buffer.from(body).toString('base64')
        console.log(databs64)
        }
    })
}
